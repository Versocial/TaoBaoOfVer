	...
	while (1) {
		...
		if (CMD == _LOGOUT) {
			...
			case logout:cout << "Already Logout.\n";
				break;
			case seller:DO(sellers->releaseFromMemoryIntoFile(userS->id())); cout << "Seller [ " << userS->id() << " ] logout.\n"; userS=NULL;
				break;
			case consumer:DO(consumers->releaseFromMemoryIntoFile(userC->id())); cout << "Consumer [ " << userC->id() << " ] logout.\n";  userC=NULL;
				break;
			default:
				break;
			}
		...
		case logout:char mod;
			...
			case 'l':cout << "Type in 's' to reach Seller MOD, or 'c' to reach Consumer MOD.\n ";
				...
				if (mod == 's') {
					...
					if (!consumers->containsInFile(id)) { cout << "Unavaliable account.\n"; continue; }
					consumers->readFromFile(id);
					userC=(Consumer*)(consumers->getObjectInMemory(id));
					cout << "Give me your password.\n";
					string passwd; cin >> passwd;// ReadByInput(passwd);
					while (!userC->matchWithPassWord(passwd)) {
